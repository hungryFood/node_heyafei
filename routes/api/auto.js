...
router.post('/submit',function(req,res,next){
  ...
  let {title,des,auth,content,oldImg,nikename,follow,fans,username,password,_id} = req.body;
  //判断传没传头像，穿了改名，没传用默认
  ...
  mgdb({collection:dataName},(collection,client,ObjectID)=>{
        collection.updateOne({_id:ObjectID(id)},{$set:{
          username,password,nikename,follow,fans,auth_icon
        }},{upsert:false,projection:true},(err,result)=>{
          if(!err && result.result.n){
            res.send("/admin/user/?dataName="+ dataName +"&start="+start+"&rule="+rule+"&q="+q+"&count="+count)
          }else{
